...
//export type CardProductProps ifadesi TypeScript'te bir tür tanımlamak için kullanılıyor.
...
const DetailClient = ({ product }: { product: any }) => {
    ...
    const increaseFunc = () => {
        if (cardProduct.quantity == 10) return //en fazla 10 olablir bunu sepete eklerken belirleniyor.
        setCardProduct(prev => ({ ...prev, quantity: prev.quantity + 1 }))
    }
    const decreaseFunc = () => {
        if (cardProduct.quantity == 1) return//birin aşağısına düşmesin
        setCardProduct(prev => ({ ...prev, quantity: prev.quantity - 1 }))
    }
            ...
            <PageContainer>
                <div className="block md:flex gap-10 justify-center">
                    <div className="relative h-[200px] md:h-[400px] w-[200px] md:w-[400px] mb-3 md:mb-0">
                        <Image src={"/pexels-pixabay-461431.jpg"} fill alt="" />
                    </div>
                    <div className="w-full md:w-1/2 space-y-3">{/* Mobil ve küçük ekranlarda: Öğenin genişliği tam olarak %100 (w-full), yani tüm ekranı kaplar.
                            Orta boyutlu ve daha büyük ekranlarda: Öğenin genişliği %50 (md:w-1/2), yani kapsayıcısının yarısını kaplar.
                            Dikey boşluklar (space-y-3): Alt öğeler arasına 12px boşluk ekler. */}
...

                        {
                            displayButton ? <>
                                ...
                                {/* counter bizden bu iki fonksiyonu istiyor. */}
